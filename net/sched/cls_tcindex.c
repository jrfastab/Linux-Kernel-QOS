...
struct tcindex_data {
	...
	struct tcindex_filter __rcu **h; /* imperfect hash; only used if !perfect
				      NULL if unused */
	struct rcu_head rcu;
	struct tcf_proto *tp;
		 ...
		 struct nlattr *est)
{
	...
	if (!cp->perfect && !cp->h) {
		if (valid_perfect_hash(cp)) {
			struct tcindex_filter_result *perfect = kcalloc(cp->hash, sizeof(*r), GFP_KERNEL);
	...
	if (r == &new_filter_result) {
		...
		for (nfp = rtnl_dereference(*fp); nfp; fp = &nfp->next, nfp = rtnl_dereference(*fp))
				/* nothing */;
